...
function displayProductInfo(product) {
    ...
    if (wishlistBtn) {
        ...
        const priceText = price > 0 ? `€${price}` : 'Цена по запросу';
        wishlistBtn.textContent = `ADD TO WISHLIST - ${priceText}`;
    }
}
...
function initializeProductPage() {
    ...
    if (wishlistBtn) {
        wishlistBtn.addEventListener('click', function() {
            ...
            alert(`Товар "${currentProduct.name}" размера ${selectedSize} добавлен в избранное!`);
        });
    }
